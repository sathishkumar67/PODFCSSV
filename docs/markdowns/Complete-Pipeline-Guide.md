...
## STEP 1: ROUND-1 LOCAL TRAINING (Per Client)

### 1.1 Extract Embeddings & Compute Local Prototypes

```python



## STEP 2: SERVER BOOTSTRAPS GLOBAL PROTOTYPES (Server)

### 2.1 Prototype Consolidation & Redundancy Removal
...
class FederatedServer:
    ...
    def __init__(self, global_model, config):
        ...
        self.global_prototypes = None  # [K_global, 768]
        self.prototype_counts = None   # [K_global]
        self.prototype_confidence = None  # [K_global]
        self.prototype_created_round = None  # [K_global]
        self.prototype_last_used = None  # [K_global]
        self.task_per_prototype = None  # [K_global] - which task proto belongs to
    
    def bootstrap_global_prototypes(self, client_protos_list, client_counts_list):
        """
        STEP 2: Server Bootstraps Global Prototypes (Round 1)
        
        Process:
        1. Collect all local prototypes from clients
        2. Concatenate (union)
        3. CRITICAL: Remove redundancy via clustering
        4. Merge similar prototypes (cosine similarity > threshold)
        5. Initialize global prototype bank
        
        Args:
            client_protos_list: List of [K_local, 768] for each client
            client_counts_list: List of [K_local] sample counts per client
        
        Returns:
            global_prototypes: [K_global, 768]
            global_counts: [K_global]
        """
        
        # Step 1: Collect & concatenate all local prototypes
        all_protos = torch.cat([p for p in client_protos_list], dim=0)  # [total, 768]
        all_counts = []
        
        for counts in client_counts_list:
            all_counts.extend(counts.tolist())
        all_counts = torch.tensor(all_counts)
        
        # Normalize
        all_protos = F.normalize(all_protos, dim=1)
        
        print(f"[Server] Received {len(all_protos)} local prototypes from {len(client_protos_list)} clients")
        
        # Step 2: CRITICAL - Remove redundancy via clustering
        # Problem: Many clients learn same concepts independently
        # Solution: Merge very similar prototypes
        ...
        for i in range(len(all_protos)):
            ...
            # Find all similar prototypes to i
            similarity_threshold = self.config.get('prototype_merge_threshold', 0.85)
            sims = F.cosine_similarity(all_protos[i:i+1], all_protos)  # [total]
            similar_indices = (sims > similarity_threshold).nonzero(as_tuple=True)[0].tolist()
            
            # Merge: weighted average by counts
            proto_merge = torch.zeros_like(all_protos[0])
            count_merge = 0
            
            for j in similar_indices:
                if j not in used_indices:
                    weight = all_counts[j].float()
                    proto_merge = proto_merge + all_protos[j] * weight
                    count_merge += weight.item()
                    used_indices.add(j)
            
            if count_merge > 0:
                proto_merge = proto_merge / count_merge
                proto_merge = F.normalize(proto_merge, dim=0)
                merged_protos.append(proto_merge)
                merged_counts.append(count_merge)
        
        self.global_prototypes = torch.stack(merged_protos)  # [K_global, 768]
        self.prototype_counts = torch.tensor(merged_counts)
        
        # Initialize metadata
        self.prototype_confidence = torch.ones(len(merged_protos))  # Will be updated
        self.prototype_created_round = torch.zeros(len(merged_protos))
        self.prototype_last_used = torch.zeros(len(merged_protos))
        self.task_per_prototype = torch.zeros(len(merged_protos), dtype=torch.long)  # Task 0 initially
        
        print(f"[Server] After redundancy removal: {len(self.global_prototypes)} global prototypes")
        
        return self.global_prototypes, self.prototype_counts
    
    def compute_prototype_confidence(self):
        """
        ENHANCEMENT 1: Compute confidence score for each prototype
        
        Confidence = (count / total_count) * (1 - normalized_variance)
        
        Higher confidence = more samples + less variance = more reliable
        """
        if self.global_prototypes is None:
            return None
        
        # Count-based confidence
        count_conf = self.prototype_counts / self.prototype_counts.sum()
        
        # Variance-based confidence (lower variance = more confident)
        # For now, assume equal variance; can be improved with more data
        variance_conf = torch.ones_like(count_conf)  # Placeholder
...
# Get outputs from all clients
all_local_protos, all_proto_counts, all_updates = federated_round_1(clients, config)

# Bootstrap global prototypes
global_protos, global_counts = server.bootstrap_global_prototypes(
    all_local_protos, all_proto_counts
)

# Compute confidence scores (Enhancement 1)
server.compute_prototype_confidence()
```

---

## STEP 3: BROADCAST GLOBAL PROTOTYPES (Server → Clients)

### 3.1 Server Message Preparation

```python
def prepare_server_broadcast(server, selected_clients):
    """
    STEP 3: Prepare message to send to clients
    
    What to send:
    - Updated adapter weights (aggregated via FedAvg)
    - Global prototypes
    - Prototype metadata (confidence, task_id)
    
    What NOT to send:
    - Full model weights (frozen backbone already at clients)
    - Raw prototype variance (metadata only)
    """
    
    # Aggregate adapter weights from selected clients
    # (This part happens after receiving client updates)
    
    server_message = {
        'round_id': current_round,
        
        # Global prototypes for distillation guidance
        'global_prototypes': server.global_prototypes,  # [K, 768] → ~3 KB
        'prototype_counts': server.prototype_counts,    # [K] → ~1 KB
        'prototype_confidence': server.prototype_confidence,  # [K] → ~1 KB
        'prototype_task_id': server.task_per_prototype,  # [K] → ~1 KB
        
        # Updated adapter weights (aggregated)
        'adapter_params': aggregated_adapters,  # ~500 KB for ViT adapters
        
        # Encoder weights NOT sent (frozen)
        'encoder_weights': None,
    }
    
    total_size = sum(
        param.numel() * 4 for param in 
        [server.global_prototypes, aggregated_adapters]
    )  # ~503 KB per round (vs 350 MB with full model)
    
    print(f"[Server] Broadcasting {total_size / 1024:.1f} KB per client")
...
## STEP 4: CLIENT ALIGNMENT (Passive) (Per Client)

### 4.1 Local-Global Prototype Alignment

```python
def client_alignment_phase(client, global_prototypes, global_confidence):
    """
    STEP 4: Client Alignment (Passive - no learning)
    
    Purpose:
    - Align local prototypes to global semantic space
    - Compute mapping: local_proto_id → global_proto_id
    - Prepare for active prototype-guided training in STEP 5
    
    Process:
    1. Compute similarity: local vs global
    2. Find best match for each local prototype
    3. Store alignment table
    4. No gradients, no model updates
    """
    
    # Compute cosine similarity matrix
    # local_protos: [K_local, 768]
    # global_protos: [K_global, 768]
    
    similarity_matrix = torch.mm(
        F.normalize(client.prototype_manager.prototypes, dim=1),
        F.normalize(global_prototypes, dim=1).T
    )  # [K_local, K_global]
    
    # Find best match for each local prototype
    best_global_idx = similarity_matrix.argmax(dim=1)  # [K_local]
    best_similarity = similarity_matrix.max(dim=1).values  # [K_local]
    
    # Store alignment table
    client.prototype_alignment = {
        'local_to_global': best_global_idx,  # Maps local idx to global idx
        'similarity': best_similarity,  # How confident is each match
        'global_confidence': global_confidence  # Download global confidence
    }
    
    print(f"[Client {client.client_id}] Aligned {len(best_global_idx)} local prototypes")
    print(f"  Average match similarity: {best_similarity.mean():.3f}")
    
    return client.prototype_alignment
```
