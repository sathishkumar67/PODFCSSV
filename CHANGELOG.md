...
- **Interleaved Stress Stages** (`main.py`): Added interleaved stress datasets, including the later Flowers102 and DTD stress stage, so the continual stream contains stronger distribution shifts before the final evaluation pass.
- **Final Probe Artifacts** (`main.py`): Completed runs now export a dedicated final linear-probe JSON summary and a per-dataset accuracy bar plot for the trained checkpoint.
...
- **Upfront Dataset Preparation** (`main.py`): Both modes now prepare every benchmark, stress, and final-probe dataset before the first training round starts so the long training loop does not pause later for first-use dataset downloads.
...
- **Cross-Domain Dataset Suite** (`new_main.py`): Replaced the earlier toy multi-dataset mix with a more diverse sequence spanning `EuroSAT`, `PCAM`, `FER2013`, `FGVC Aircraft`, `DTD`, `Oxford-IIIT Pet`, `Flowers102`, `Food101`, `GTSRB`, and `SVHN`.
