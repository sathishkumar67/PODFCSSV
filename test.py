...
CONFIG = {
    ...
    # GTSRB has 43 classes. We will do 4 rounds.
    # Rounds 1-3 will have 11 classes, Round 4 will have 10 classes.
    "num_rounds": 4,
    "classes_per_task": 11,

    # Number of classes assigned to each client per task (non-IID split).
    # If a task has 11 classes and 2 clients, we can give e.g. 5 to each, leaving
    # some unused or overlapping. We'll simply use 5 classes per client to be safe.
    "classes_per_client": 5,

    # Root directory for the GTSRB dataset.
...
# ==========================================================================
# DATA PIPELINE — GTSRB (Autonomous Driving OOD) with Task Scheduling
# ==========================================================================


def load_gtsrb(data_root: str, image_size: int = 224):
    """
    Load the GTSRB training dataset with transforms.

    GTSRB (German Traffic Sign Recognition Benchmark) contains 43 classes of
    traffic signs. Images are of varying sizes and are resized to ``image_size`` 
    (224) to match the ViT-MAE's expected input resolution.
    ...
    -------
    datasets.GTSRB
        The full GTSRB training dataset with transforms applied.
    """
    ...
    logger.info(f"Loading/Downloading GTSRB to {data_root}...")
    ...
    # GTSRB does not natively expose a .classes list like ImageFolder.
    # Create an artificial classes list based on its 43 classes.
    dataset.classes = [str(i) for i in range(43)]
    
    logger.info(
        f"Loaded GTSRB: {len(dataset)} images, "
...
def create_task_schedule(
    ...
    Splits the 43 GTSRB classes into sequential tasks, then
    ...
    for round_idx in range(num_rounds):
        ...
        # For the final task, handle leftover classes correctly
        end = min(start + classes_per_task, len(shuffled_classes))
        task_classes = shuffled_classes[start:end]

        # Divide task classes across clients (non-IID).
        # We ensure we don't exceed the available classes in the task slice
        round_schedule = []
        c_idx = 0
        for client_idx in range(num_clients):
            c_start = c_idx
            c_end = min(c_start + classes_per_client, len(task_classes))
...
def create_client_dataloaders(
    ...
    ----------
    dataset : GTSRB
        The full GTSRB training dataset.
    client_classes : List[List[int]]
    ...
    # GTSRB provides dataset._samples as a list of (path, label) tuples
    if hasattr(dataset, '_samples'):
        targets = torch.tensor([s[1] for s in dataset._samples])
    else:
